...
export class UsuarioCardComponent implements OnInit{
  ...
  @Input() usuarioSeleccionado!: UsuarioPaciente | UsuarioEspecialista |   Usuario;
  ...
  ngOnInit(): void {
    ...
    this.ultimos3TurnosTomados = this.turnosSrv.listadoTurnos
                                .filter(turno => (turno.paciente) && turno.paciente!.id == this.usuarioSeleccionado!.id)
    ...
    if(this.usuarioSeleccionado.perfil == Perfil.especialista){
      this.especialidades = (this.usuarioSeleccionado as UsuarioEspecialista).especialidades;
      this.usuarioEspecialista = this.usuarioSeleccionado as UsuarioEspecialista;
      this.botonEspecialista = this.usuarioEspecialista.cuentaAprobada ? 'Cuenta aprobada' : 'Aprobar cuenta';
    }
    else{
  ...
  tieneCuentaAprobada() {
    return this.usuarioSeleccionado?.perfil == Perfil.especialista && (this.usuarioSeleccionado as UsuarioEspecialista).cuentaAprobada;
  }
  ...
  tieneHistoriaClinica(){
    return this.historiasClinicasSrv.lstHistoriasClinicas.some(h => h.turno.paciente?.id == this.usuarioSeleccionado?.id )
  }
  ...
  exportarTurnos(){
    ...
    const turnos = this.turnosSrv.listadoTurnos.filter(turno =>
                                    (this.usuarioSeleccionado?.perfil == Perfil.especialista && turno.especialista?.id == this.usuarioSeleccionado?.id) ||
                                    (this.usuarioSeleccionado?.perfil == Perfil.paciente && turno.paciente?.id == this.usuarioSeleccionado?.id)
                                    );
    ...
    workbook.xlsx.writeBuffer().then(buffer => {
      this.guardarArchivoExcel(buffer, `Turnos_${this.usuarioSeleccionado?.perfil}_${this.usuarioSeleccionado?.apellido}_${this.usuarioSeleccionado?.nombre}_${this.fechaArchivo()}.xlsx`);
    });
  }

  generarHojaTurnosPerfil(workbook: ExcelJS.Workbook, turnos: Turno[]): void {
    const perfil = this.usuarioSeleccionado?.perfil;
    const worksheet = workbook.addWorksheet(`Turnos ${perfil == Perfil.especialista ? 'Dados' : 'Tomados'}`);
    ...
    turnos.forEach((turno: any) => {
      ...
      if (perfil === Perfil.paciente) {
        row.push(turno.especialista.nombre + ', ' +turno.especialista.apellido);
      } else if (perfil === Perfil.especialista) {
